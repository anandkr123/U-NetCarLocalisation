...
# a ticker for copying the value in array for the first time
ticker_train = 1
ticker_test = 1
...
for data in myObj_train:
    ...
    # creating the masks of the image, |DONE ONLY ONCE|
    # center = np.array([xtemp, ytemp])
    # mask = create_circular_mask(IMG_HEIGHT, IMG_WIDTH, center, 16)
    # masked_img = mask.copy()
    # new_mask = masked_img * 255
    # cv2.imwrite(mas_path, new_mask)
    # count += 1
...
net = conv2d(x, 32, 1, "Y0", strides=(1, 1))

net = conv2d(net, 64, 3, "Y1", strides=(2, 2))

net = conv2d(net, 128, 3, "Y2", strides=(2, 2))
